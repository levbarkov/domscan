	...
	function addImage(ind) {
		...
		for (i = start; i < end; i++) {
			...
			if($('#dropped-files > .image').length <= maxFiles) { 
				$('#dropped-files').append('<div id="img-'+i+'" class="image" style="background: url('+dataArray[i].value+'); background-size: cover;"> <a href="#1" id="drop-'+i+'" class="drop-button">Удалить изображение</a><input name="Images[]" type="hidden" value="'+dataArray[i].value+'">"</div>'); 
			}
		}
		return false;
	}

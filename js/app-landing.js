...
var MapMarkerService = (function () {
    ...
    _createClass(MapMarkerService, [{
        ...
        key: 'newMarker',
        value: function newMarker(data) {
            var markerIcon = DG.icon({
                iconUrl: '/img/icons/on_map_many.png',
                iconRetinaUrl: '/img/icons/on_map_many.png',
                iconSize: [40, 55],
                iconAnchor: [20, 55],
                shadowUrl: '/img/icon-map-shadow.png',
                shadowRetinaUrl: '/img/icon-map-shadow@2x.png',
            ...
            c += 'normal onmapsingle';

			//console.log(data);
            //html = '<div><span>'+data.for_sale+'</span></div>';
            ...
            //var marker = DG.marker([data.lat, data.long], {icon: markerIcon}).addTo(map);
        ...
        key: 'createNewMarker',
        value: function createNewMarker(data) {
            var marker;
            marker = self.newMarker(data);
            markerCategory++;
...
var MapPopupService = (function () {
    ...
    _createClass(MapPopupService, [{
        ...
        value: function createPopup(marker, data) {
			...
			if (data.type == 'new')
				html = '<div class=""></div>\n      <img src="'+ data.image +'" style="max-width:100%">\n      <a href="' + data.uri + '" class="balloon-title">' + data.title + '</a>\n            <p class="balloon-field"><span class="balloon-field-name">Адрес: </span>\n            <span class="balloon-field-value">' + data.address + '</span></p>\n            <p class="balloon-field"><span class="balloon-field-name">Квартиры на продажу: </span>\n            <span class="balloon-field-value"> ' + data.for_sale + '</span></p>\n            <p class="balloon-field"><span class="balloon-field-name">Сдача: </span>\n            <span class="balloon-field-value">' + data.year + '</span></p>\n            <p class="balloon-cost">от ' + data.cost + ' руб./м2</p>';
        ...
        value: function createClusterMarker(cluster) {
            ...
            //console.dir(cluster.stats);

            if (cluster.stats[1] === 0 && cluster.stats[2] === 0) {
                c += 'normal';
                size = {
                    height: 50,
                    width: 28
            ...
            //if only special
            else if (cluster.stats[2] > 0 /* */) {
                c += 'special onmapmultiple';
                size = {
                    height: 64,
                    width: 38
                };
    ...
    _createClass(MapService, [{
            ...
            if (data.items != null) {
                $.each(data.items, function (index, value) {
                    ...
                    if (!(value.lat != null && value.long != null && value.lat != 0 && value.long != 0)) {} else {
						...
						if(value.type == 'new')
                        	var marker = MAPMARKERSERVICE.get(self).createNewMarker(value);
                        else                        
        ...
        value: function showMap() {
            if (LOCATIONSERVICE.get(self).get().view === 'map') {
                ...
                $('#accordion-wrapper').height(window.innerHeight - 140);
            }
        }
    }, {
        key: 'hideMap',
        ...
        value: function resizeMap() {
            try {
                ...
                self.map.fitBounds(bounds, {
                    ...
                    paddingBottomRight: [mapX, 100]
                });

     //           self.map.fitBounds(bounds);

                $('.search__map-special-wrapper').css('margin-top', window.innerHeight - 182);
...
var menu = (function () {
    ...
    _createClass(menu, null, [{
        ...
        value: function init() {
            ...
            $(".chosen-drop").mouseenter(function(){     
			
			   $("#accordion-wrapper").mCustomScrollbar("disable");  
			
			}).mouseleave(function(){
			
			   $("#accordion-wrapper").mCustomScrollbar("update");

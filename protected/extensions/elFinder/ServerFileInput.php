...
//        $cs->registerScriptFile('http://ajax.googleapis.com/ajax/libs/jqueryui/1.8.21/jquery-ui.min.js');
    ...
    public function run() {
        ...
        if ($this->id_target) {
            $name = 'Images[' . $this->id_target . '][image]';
            $id.=$this->id_target;
            $this->class = 'span4';
        }
        ...
        if (!$this->id_target) {
            if ($this->hasModel())
                echo CHtml::activeLabel($this->model, $this->attribute) . '<br> ';
            else
                echo $this->label;
        }
        $inputOptions = array('id' => $id, 'class' => $this->class, 'name' => $name, /* 'style' => 'float:left;' , 'readonly' => 'readonly' */);
        if ($this->hasModel())
            echo CHtml::activeTextField($this->model, $this->attribute, $inputOptions);
        else
            echo CHtml::textField($name, $this->value, $inputOptions);

        echo CHtml::button('Выбрать', array('id' => $id . 'browse', 'class' => 'btn span'));

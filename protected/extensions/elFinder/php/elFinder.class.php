					...
					/*	$allowedTypes = array(IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_GIF);
						...
						if(!$error) {
							if (strtolower($file_parts['extension']) == 'png') {
								...
								$watermark = imagecreatefrompng($_SERVER['DOCUMENT_ROOT'].'/files/watermark.png');
								$watermarkSize = getimagesize($_SERVER['DOCUMENT_ROOT'].'/files/watermark.png');

								if ($imageSize[0] < $imageSize[1]) {
									$newWatermarkWidth = $imageSize[0];
									$ratio_orig = $watermarkSize[0]/$watermarkSize[1];
									$newWatermarkHeight = $newWatermarkWidth/$ratio_orig;
								} else {
									$newWatermarkHeight = $imageSize[1];
									$ratio_orig = $watermarkSize[0]/$watermarkSize[1];
									$newWatermarkWidth = $newWatermarkHeight/$ratio_orig;
								}

								$success = imagecopyresampled($image,                 // Destination image
								   $watermark,                              // Source image
								   ($imageSize[0]-$newWatermarkWidth)/2,  // Destination X
								   ($imageSize[1]-$newWatermarkHeight)/2, // Destination Y
								   0,                                       // Source X
								   0,                                       // Source Y

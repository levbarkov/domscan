...
class newTCPDF extends TCPDF
{
    ...
    public function Footer() {
        ...
        // Page number
        $this->Cell(0, 10, 'Распечатано с сайта domscan.ru', 0, false, 'C', 0, '', 0, false, 'T', 'M');
    }
}
...
class ETcPdf
{
   ...
   public function __construct($orientation, $unit, $format, $unicode, $encoding)
   {
      ...
      if (!is_bool($unicode))
		...
		$this->myTCPDF = new newTCPDF($orientation, $unit, $format, $unicode, $encoding);
    ...
    */
	...
	{
		if (is_object($this->myTCPDF) && get_class($this->myTCPDF)==='newTCPDF') return call_user_func_array(array($this->myTCPDF, $method), $params);
		else throw new CException(Yii::t('ETcPdf', 'Can not call a method of a non existent object'));
	}

	public function __set($name, $value)
	{
	   if (is_object($this->myTCPDF) && get_class($this->myTCPDF)==='newTCPDF') $this->myTCPDF->$name = $value;
	   else throw new CException(Yii::t('ETcPdf', 'Can not set a property of a non existent object'));
	}

	public function __get($name)
	{
	   if (is_object($this->myTCPDF) && get_class($this->myTCPDF)==='newTCPDF') return $this->myTCPDF->$name;

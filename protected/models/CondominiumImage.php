...
class CondominiumImage extends CActiveRecord
{
	...
	public static function saveImages($images, $id, $taxonomy)
	{
		...
		if (isset($images) AND (!empty($images[0]))) {
			$checkdir = dirname(Yii::app()->request->scriptFile).'/uploads/condos/';
			if (!is_dir($checkdir)) {
				mkdir($checkdir);
			}
			...
			foreach ($images as $image) {
				...
				if($count_already_uploaded < 6) {
					...
					if(file_put_contents($uploaddir.$randomName_mime, $decodedData)) {
						...
						$magicianObj -> resizeImage(1780, 1000, 'auto');
						...
						$item = CondominiumImage::model()->findByAttributes(array('parent'=>$id,'main'=>1));
						if($item)
							$is_main = false;
						else {
							$is_main = true;
							$magicianObj = new imageLib($uploaddir.$randomName_mime);
							$magicianObj -> resizeImage(460, 300, 'crop');
							$magicianObj -> saveImage($uploaddir.'main_image.jpeg');
							$estate = Condominium::model()->findByPk($id);
							$estate->image = '/uploads/condos/item_'. $id . '/main_image.jpeg';
							$estate->save();
						}
	...
	{
		$item = Condominium::model()->findByPk($itemId);
	...
	public static function deleteImage($imageId, $itemId)
	{
		if (self::deleteImageFile($imageId, $itemId)) {
			...
			$estate = Condominium::model()->findByPk($itemId);
	...
	{	
		...
		if ($image) {
			$command = Yii::app()->db->createCommand();
			$command->update('real_condominium_image', array(
			    'main'=>'false',
			...
			if (file_exists($filename)) {
				...
				$magicianObj -> resizeImage(460, 300, 'crop',true);

            ...
            <div class="property__wrapper">
                ...
                <div class="row">
                    ...
                    <div class="col-xs-4 ">
						...
						<? endif ?>
						<? 	switch($model->type) {
								case 'flat': $type_string = 'квартиры'; break;
								case 'land': $type_string =  'земельные участки'; break;
								case 'commercial': $type_string =  'коммерческую недвижимость'; break;
								case 'new': $type_string =  'новостройки'; break;
								case 'house': $type_string =  'дома'; break;
							}
						
						?>
						<span class="property__help-title">Посмотреть на карте:</span>
                        <ul class="property__help-list">
	                        <? $citytitle = ''; $city = City::model()->findByPk($model->city); if($city) $citytitle = $city->name; ?>
							<li class="property__help-list-item"><a class="property__help-list-item-link" href="/catalog/<?= $model->type ?>/#?view=table&type=<?= $model->type ?>&subtype=<?= $model->subtype ?>">Другие <?= $type_string ?> в <?= $citytitle ?>е</a></li>
	                        <? $citytitle = ''; $city = District::model()->findByPk($model->district); if($city) $citytitle = $city->name; ?>
							<li class="property__help-list-item"><a class="property__help-list-item-link" href="/catalog/<?= $model->type ?>/#?view=table&type=<?= $model->type ?>&subtype=<?= $model->subtype ?>">Другие <?= $type_string ?> в <?= $citytitle ?></a></li>
                        </ul>
						<?

<?
	...
	$months_title = array('','Январь','Февраль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь');
		...
		if($selected_month && $selected_year):
					$this->setPageTitle('Гайды '.$months_title[$selected_month].' '.$selected_year);
                      
                 else: 
				 	$this->setPageTitle('Гайды');
                 endif
?>
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-xs-12">
                ...
                <? if($selected_month && $selected_year): ?>
                    <h1>Гайды <?= $months_title[$selected_month] ?> <?= $selected_year ?></h1>
                <? else: ?>
                    <h1>Гайды</h1>
                <? endif ?>
                </div>
                ...
                <div class="news__wrapper">
                    ...
                    <div class="news__content-wrapper">
                        <h2 class="news__content-title">
                            <a href="<?= $news->getViewURL() ?>"><?= $news->title ?></a>
                        </h2>
                        <p><?= strip_tags($news->additional) ?></p>
                        <span class="news__content-more"><a href="<?= $news->getViewURL() ?>">Читать</a></span>
                    </div>

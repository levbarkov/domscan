...
<a href="/news/create" class="btn btn-warning"><i class="icon-plus icon-white"></i> Добавить новость</a>
<div class="block">
    ...
    <div class="block-content collapse in">
    	<table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
              <td><?= $otziv->title ?></td>
              <td><? foreach($otziv->tags as $tag) { echo $tag->title.', '; } ?></td>
              <td><?= $otziv->datepublish ?></td>
              <td>
              	<nobr><a class="btn btn-info" href="/news/update/<?= $otziv->id ?>"><i class="icon-pencil icon-white"></i></a>
              	<a class="btn btn-danger" href="/news/delete/<?= $otziv->id ?>" onclick="return confirm('Вы точно хотите удалить? Эту операцию отменить нельзя!')"><i class="icon-remove icon-white"></i></a></nobr>
              </td>
            </tr>
            <? endforeach ?>
          </tbody>
        		...
        		<div class="dataTables_paginate paging_bootstrap pagination" style="float:right;">
        			<ul>
        				...
        				<? for($i = 1; $i <= $page_count; $i++): ?>
        					<? 
	        					$assemble_link = '/news/admin/page/'.$i;        					

...
class ZastroischikiPagesController extends Controller
{
	...
	public function actionView()
	{
		...
		$en_title = Yii::app()->request->getParam('en_page_title', '0');		
		$parent_title = Yii::app()->request->getParam('en_title', '0');
		
		$builder = Builder::model()->findByAttributes(array('en_title'=>$parent_title));
		if (!$builder)
			throw new CHttpException(404,'The requested page does not exist.');
			
		$model = BuilderStaticPage::model()->findByAttributes(array('parent_id' => $builder->id, 'en_title'=>$en_title));
	...
	public function actionAdmin()
	{
		...
		$model = BuilderStaticPage::model()->findAllByAttributes(array('parent_id'=>$builder->id)); 
	...
	public function actionCreate()
	{
		...
		$model->parent_id = $builder->id;
		
		//получаем список жанров
		//$types = Types::model()->findAll();
		//если получены данные игры
		if(isset($_POST['BuilderStaticPage']))
		{
			//записываем атрибуты
			$model->attributes=$_POST['BuilderStaticPage'];
	...
	public function loadModel()
	{
		if($this->_model===null)
		{
			...
			{
				$condition='';
				$this->_model=BuilderStaticPage::model()->findByPk($_GET['id'], $condition);
			}
			if($this->_model===null)
				throw new CHttpException(404,'The requested page does not exist.');
		}

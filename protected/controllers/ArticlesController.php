...
class ArticlesController extends Controller
{
	...
	public function actionAdmin()
	{
		...
		$model_count = Articles::model()->count(
			$criteria
		); 
	...
	public function actionView()
	{
		...
		$this->render('//news/view',array(
			'model'=>$model, 'pagination_url'=>'/articles', 'title'=>'Статьи', 'show_menu'=>false
		));
	}
	...
	public function saveImage($model) 
	{
		if($model->save()){	
			...
			if($uploaded_file) {
						$checkdir = dirname(Yii::app()->request->scriptFile).'/uploads/articles/';
						if (!is_dir($checkdir)) {
							mkdir($checkdir);
						}
						...
						$uploaded_file->saveAs(dirname(Yii::app()->request->scriptFile).'/uploads/articles/'.$filename.'_image.'.$fileextension);
						
						$model->photo = '/uploads/articles/'.$filename.'_image.'.$fileextension; 							
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Articles']))
		{
			...
			if($model->save()) {
				$this->redirect(array('articles/'.$model->id));
			}
	...
	public function actionIndex()
	{
		...
		$count = Articles::model()->countByAttributes($attribs, $criteria);
		$criteria->limit = 8;
		$criteria->offset = $criteria->limit * ($current_page-1);
		$page_count = $count / $criteria->limit;
			
		$model = Articles::model()->findAllByAttributes($attribs, $criteria);
		//(array('datepublish'=>''),array('order'=>'datepublish DESC'));
		$this->render('//news/index',array(
			'news_list'=>$model, 'page_count'=>$page_count, 'pagination_url'=>'/articles', 'current_page'=>$current_page, 'title'=>'Статьи', 'show_menu'=>false));
	}
	...
	public function loadModel()
	{
		...
		{
			if(isset($_GET['id']))
				$this->_model=Articles::model()->findbyPk($_GET['id']);
			if($this->_model===null)
